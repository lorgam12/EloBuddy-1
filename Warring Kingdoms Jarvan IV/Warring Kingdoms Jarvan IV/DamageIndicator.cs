...
namespace Warring_Kingdoms_Jarvan_IV
{
    ...
    public static class DamageIndicator
    {
        ...
        public static void Initialize(DamageToUnitDelegate damageToUnit)
        {
            ...
            HealthbarEnabled = JarvanMenu.Draw["DrawDMG"].Cast<CheckBox>().CurrentValue;
            PercentEnabled = true;
            /*
            ...
            if (HealthbarEnabled || PercentEnabled)
            {
                foreach (var unit in EntityManager.Heroes.Enemies.Where(u => u.IsHPBarRendered))
                {
                    ...
                    if (PercentEnabled)
                    {
                        ...
                        Drawing.DrawText(new Vector2(unit.HPBarPosition.X, unit.HPBarPosition.Y + 20), Color.Black, string.Concat(Math.Ceiling((damage / unit.TotalShieldHealth()) * 100), "%"), 10);
                    }
                }
            }
        }
